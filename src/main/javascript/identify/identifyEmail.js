...
/**
 * An identifyEmail object
 * @class
 */
 IDENTIFYAPIS.identifyEmail = function(accessToken){
	IDENTIFYAPIS.baseService.call(this, accessToken);
};
/**
 * identifyEmail inherits baseService
 * @class
 */
...
/**
 * Set API URL to post forward identifyEmail, it will call corresponding url(xml or json) based on type of postData(input request body)
 * @param postData {string} request body
 * @param callback {string} method name (optional)
 * @return response object or calls callback
 */
IDENTIFYAPIS.identifyEmail.prototype.validateEmailAddress = function(postData, callback){

	var isJson = this.isJson(postData);
	if(isJson){
		var apiUrl = '/identifyemail/v1/rest/validateemailaddress/results.json';
		...
		if(isXml){
			var apiUrl = '/identifyemail/v1/rest/validateemailaddress/results.xml';
